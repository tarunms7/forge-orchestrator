...
class TestAwaitingInputTransitions:
    def test_in_progress_to_awaiting_input(self):
        assert TaskStateMachine.transition(TaskState.IN_PROGRESS, TaskState.AWAITING_INPUT) == TaskState.AWAITING_INPUT

    def test_awaiting_input_to_in_progress(self):
        assert TaskStateMachine.transition(TaskState.AWAITING_INPUT, TaskState.IN_PROGRESS) == TaskState.IN_PROGRESS

    def test_awaiting_input_to_cancelled(self):
        assert TaskStateMachine.transition(TaskState.AWAITING_INPUT, TaskState.CANCELLED) == TaskState.CANCELLED

    def test_awaiting_input_to_error(self):
        assert TaskStateMachine.transition(TaskState.AWAITING_INPUT, TaskState.ERROR) == TaskState.ERROR
...
class TestBlockedTransitions:
    ...
    def test_blocked_to_cancelled(self):
        assert TaskStateMachine.transition(TaskState.BLOCKED, TaskState.CANCELLED) == TaskState.CANCELLED
...
class TestCanTransition:
    ...
    def test_awaiting_input_to_in_progress_returns_true(self):
        assert TaskStateMachine.can_transition(TaskState.AWAITING_INPUT, TaskState.IN_PROGRESS) is True
    ...
    def test_in_progress_to_awaiting_input_returns_true(self):
        assert TaskStateMachine.can_transition(TaskState.IN_PROGRESS, TaskState.AWAITING_INPUT) is True
