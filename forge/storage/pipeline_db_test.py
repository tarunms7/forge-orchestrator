...
async def test_get_tasks_by_ids_basic(db):
    """get_tasks_by_ids returns matching tasks for valid IDs."""
    await db.create_task(
    ...
    tasks = await db.get_tasks_by_ids(["t1", "t3"])
    assert len(tasks) == 2
    ids = {t.id for t in tasks}
    assert ids == {"t1", "t3"}


async def test_get_tasks_by_ids_empty_input(db):
    """get_tasks_by_ids with empty list returns empty list without DB query."""
    result = await db.get_tasks_by_ids([])
    assert result == []


async def test_get_tasks_by_ids_missing_ids(db):
    """get_tasks_by_ids silently omits missing IDs."""
    await db.create_task(
        id="t1", title="T1", description="D", files=[], depends_on=[], complexity="low"
    )
    tasks = await db.get_tasks_by_ids(["t1", "nonexistent"])
    assert len(tasks) == 1
    assert tasks[0].id == "t1"


async def test_get_tasks_by_ids_all_missing(db):
    """get_tasks_by_ids returns empty list when all IDs are missing."""
    tasks = await db.get_tasks_by_ids(["nope1", "nope2"])
    assert tasks == []


async def test_get_tasks_by_ids_dedup(db):
    """Duplicate IDs in input produce only one TaskRow per unique ID."""
    await db.create_task(
        id="t1", title="T1", description="D", files=[], depends_on=[], complexity="low"
    )
    tasks = await db.get_tasks_by_ids(["t1", "t1", "t1"])
    assert len(tasks) == 1
    assert tasks[0].id == "t1"


# ── get_pipeline_analytics limit test ────────────────────────────────


async def test_get_pipeline_analytics_respects_limit(db):
    """get_pipeline_analytics with a custom limit returns only that many pipelines' data."""
    for i in range(5):
        await _create_pipeline_with_status(db, f"p{i}", "done")

    # Default (no limit) should include all
    result = await db.get_pipeline_analytics()
    assert result["total"] == 5
    assert result["passed"] == 5

    # With limit=3, only the 3 most recent should be counted
    result = await db.get_pipeline_analytics(limit=3)
    assert result["total"] == 3
    assert result["passed"] == 3


# ── get_pipeline_export_data SQL optimization test ───────────────────


async def test_get_pipeline_export_data_returns_correct_data(db):
    """Verify export data still returns correct task data after column-select optimization."""
    await db.create_pipeline(
        id="pipe-opt", description="Optimization test", project_dir="/tmp", model_strategy="auto"
    )
    await db.create_task(
        id="t1",
        title="Optimized task",
        description="Testing column select",
        files=["a.py", "b.py"],
        depends_on=[],
        complexity="high",
    ...
    await db.add_task_agent_cost("t1", cost=0.50, input_tokens=5000, output_tokens=2000)

    data = await db.get_pipeline_export_data("pipe-opt")
    assert len(data["tasks"]) == 1
    t = data["tasks"][0]
    assert t["id"] == "t1"
    assert t["title"] == "Optimized task"
    assert t["state"] == "done"
...
# ── purge_old_pipelines optimized cascade test ───────────────────────


async def test_purge_old_pipelines_cascade_with_subquery(db):
    """Verify purge still correctly cascade-deletes tasks after subquery optimization."""
    from datetime import UTC, datetime, timedelta
    ...
    await db.create_pipeline(
        id="old-p", description="Old", project_dir="/tmp", model_strategy="auto"
    )
    await db.create_task(
        id="t-old-1", title="T1", description="D", files=[], depends_on=[],
        complexity="low", pipeline_id="old-p",
    )
    await db.create_task(
        id="t-old-2", title="T2", description="D", files=[], depends_on=[],
        complexity="low", pipeline_id="old-p",
    )
    ...
    async with db._session_factory() as session:
        await session.execute(
            update(PipelineRow).where(PipelineRow.id == "old-p").values(created_at=old_date)
        )
        await session.commit()

    deleted = await db.purge_old_pipelines(older_than_days=30)
    assert deleted == 1
    assert await db.get_pipeline("old-p") is None
    assert await db.get_task("t-old-1") is None
    assert await db.get_task("t-old-2") is None
