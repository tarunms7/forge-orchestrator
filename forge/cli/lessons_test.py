...
def test_lessons_add_no_duplicate():
    ...
    with patch("forge.cli.lessons._get_db", return_value=db):
        result = runner.invoke(lessons, [
            "add", "--global", "--category", "command_failure",
            "My Lesson", "Fix it this way",
        ])
...
def test_lessons_add_with_trigger():
    ...
    with patch("forge.cli.lessons._get_db", return_value=db):
        result = runner.invoke(lessons, [
            "add", "--global", "--category", "code_pattern",
            "--trigger", "custom trigger pattern",
            "My Lesson", "Fix it this way",
        ])
    assert result.exit_code == 0
