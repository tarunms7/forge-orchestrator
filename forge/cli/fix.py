...
@click.option(
    "--dry-run", is_flag=True, help="Plan only — print composed prompt and plan, then exit"
)
...
def fix(
    ...
    if branch_result.returncode != 0:
        click.echo(
            f"Error: failed to create branch '{branch_name}': {branch_result.stderr.strip()}"
        )
        raise SystemExit(1)
...
async def _plan_only(daemon, prompt: str):
    ...
    try:
        ...
        await db.create_pipeline(
            id=pipeline_id,
            description=prompt,
            project_dir=daemon._project_dir,
            model_strategy=daemon._strategy,
